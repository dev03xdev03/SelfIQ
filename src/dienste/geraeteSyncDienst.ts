...
export const linkDeviceToAccount = async (
  ...
  try {
    ...
    const { data: deviceCheck } = await (supabase as any)
      .from('users')
      .select('id')
      .eq('device_id', deviceId)
      .single();
    ...
    await AsyncStorage.setItem('@selfiq_user_id', existingUser.id);
...
export const autoLoginWithDevice = async (): Promise<{
  ...
  try {
    ...
    // Speichere komplette User-Daten für Timer Badge und andere Features
    await AsyncStorage.setItem('@selfiq_user', JSON.stringify(userData));

    // Speichere Username lokal für CategoryScreen (Backward Compatibility)
    await AsyncStorage.setItem('@selfiq_username', userData.username);
