...
import { EpisodeProgress } from '../hilfsmittel/fortschrittSpeicher';
...
export const saveProgressToSupabase = async (
  ...
  try {
    ...
    const { error } = await (supabase as any).from('story_progress').upsert(
      {
        user_id: user.id,
...
export const saveChoiceToSupabase = async (
  ...
  try {
    ...
    await (supabase as any).from('user_choices').insert({
      user_id: user.id,
      story_id: storyId,
      scene_id: sceneId,
      choice_id: choiceId,
